...
class DispeceriScreen(Screen):
    ...
    def _pozovi(self, stavka_id):
        s = DISPECERI.nadji(stavka_id)
        if not s or not s.get("telefon"):
            return
        try:
            webbrowser.open(f"tel:{s['telefon']}")
        except Exception as e:
            _prikazi_popup_poruku("Greska", f"Ne mogu da pokrenem poziv:\n{e}")
    ...
    def _promeni_smenu(self, stavka_id):
        ...
        s["smena"] = nova_smena
        DISPECERI.azuriraj(app.user_data_dir, stavka_id, s)
        self.ucitaj_dispecere()
    ...
    def _obrisi(self, stavka_id):
        app = App.get_running_app()
        DISPECERI.obrisi(app.user_data_dir, stavka_id)
        self.ucitaj_dispecere()
    ...
    def sacuvaj_dispecera(self):
        ...
        if self.izmena_id is not None:
            DISPECERI.azuriraj(app.user_data_dir, self.izmena_id, stavka)
            self.izmena_id = None
            self.dugme_tekst = "Dodaj dispecera"
        else:
            DISPECERI.dodaj(app.user_data_dir, stavka)
...
class TaksiApp(App):
    ...
    def build(self):
        ...
        try:
            ...
            SMENE.ucitaj(self.user_data_dir)
